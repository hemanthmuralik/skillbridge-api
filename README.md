...
| | |
|---|---|
| **Base URL** | [https://skillbridge-api-sh5w.onrender.com](https://skillbridge-api-sh5w.onrender.com) |
| **Interactive docs** | [`https://skillbridge-api.onrender.com/docs`](https://skillbridge-api-sh5w.onrender.com) |
...
**The `SECRET_KEY` defaults to a hardcoded dev value** (`"dev-secret-key-change-in-production"`) if the environment variable is missing. In production this means an attacker who reads the source could forge arbitrary JWTs.

**Fix with more time:** Remove the default fallback entirely — raise a `ValueError` at startup if `SECRET_KEY` is not set or is shorter than 32 characters. Add a pre-flight check in `main.py` that refuses to start without a secure key. Additionally, enforce `SECRET_KEY` rotation via CI checks.
...
| Deployment to Render | ✅ (update URL above after deploy) |
| Token revocation | ❌ Stateless — see "rotation and revocation" section above |
| Pagination on monitoring endpoint | ❌ Skipped for scope |
...
I'd move the attendance-rate calculation out of the route handler and into a dedicated SQL query using `GROUP BY` and `COUNT` rather than loading all records into Python and iterating. At small seed-data scale it's invisible, but `/programme/summary` does N+1 queries across institutions → batches → sessions. A single CTE or subquery would make it O(1) round-trips regardless of scale.
